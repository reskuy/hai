    ...
    <v-dialog v-model="dialog" width="380px" persistent>
        <v-card>
            <v-toolbar color="#a10115">
                <span class="text-button" style="color:white;" dark>Export Excel Tes Drive</span>
                <v-spacer/>
...
export default {
    ...
    mounted(){
        Vue.prototype.$OpenLaporanTesDrive = this.dialogx
    },
    watch:{
        // TglAwal(x){
        //     if(x != null){
    ...
    methods:{
        ...
        GenerateReport(){
            this.dialog = false
            API.get('laporantesdrive/'+this.TglAwal+'/'+this.TglAkhir).then(x=>{
                console.log(x)
               const xlsx = require('xlsx');
                ...
                if(this.filter == 'Kembali'){
                    x.data.forEach(z => {
                        ...
                        newArray.push({
                        'Tanggal Pemakaian': this.$DateConvert(z.tanggal_pemakaian),
                        ...
                        'Jam Masuk':z.pengembalian == null ? ' ' : z.pengembalian.jam_masuk,
                        'Nama Customer':z.nama_customer,
                        'Lokasi TesDrive':z.lokasi_tes_drive,
                        })
                      }
                    })
                }
                ...
                xlsx.writeFile(wb, 'Laporan '+this.filter+' TesDrive '+this.$DateConvert(this.TglAwal)+'-'+this.$DateConvert(this.TglAkhir)+'.xlsx');
            })
        }
    }
}
