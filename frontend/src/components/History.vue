...
<v-card height="100hv">
  ...
  >
    <template v-slot:top>
      ...
      dark
      >
        <v-toolbar-title>Data History</v-toolbar-title>
        <v-divider
          class="mx-4"
          vertical
        ></v-divider>
...
import API from "@/services/http";
  export default {
    ...
    methods: {
      ...
      getDataPeminjaman(){
        let department = JSON.parse(localStorage.getItem('departmentlogged'))
        this.$loading(true)
        API.get("/historypeminjaman/"+department.id_department).then(x=>{
